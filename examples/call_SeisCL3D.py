...
model={}
model['vp']=np.zeros( (csts['N'][0],csts['N'][1],csts['N'][2]))+3500  #Must contain the variables names of the chosen paretrization
model['vs']=np.zeros( (csts['N'][0],csts['N'][1],csts['N'][2]))+2000
model['rho']=np.zeros( (csts['N'][0],csts['N'][1],csts['N'][2]))+2000
model['taup']=np.zeros( (csts['N'][0],csts['N'][1],csts['N'][2]))+0.02
model['taus']=np.zeros( (csts['N'][0],csts['N'][1],csts['N'][2]))+0.02
...
for ii in range(0,csts['N'][2]-2*csts['nab']-10,60):
    ...
    for jj in range(0,csts['N'][2]-2*csts['nab']-10):
        toappend=np.zeros((8,1))
        toappend[0,:]=(csts['N'][2] - csts['nab']-5)*csts['dh']
        toappend[1,:]=(csts['N'][1]//2)*csts['dh']
...
NZ, NY, NX = csts['N']=np.array([64,64,64])
#________________Launch simulation______________
model['vp'][(NZ//2-5):(NZ//2+5),(NY//2-5):(NY//2+5),(NX//2-5):(NX//2+5)]= 4000
...
model['vp'][(NZ//2-5):(NZ//2+5),(NY//2-5):(NY//2+5),(NX//2-5):(NX//2+5)]= 3500
