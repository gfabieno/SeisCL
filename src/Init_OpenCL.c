    ...
    for (d=0; d<m->NUM_DEVICES; d++) {
        ...
        *  size, and its location in the domain*/
        ...
        //Set the sources and receivers structure for this device
        {
            ...
            __GUARD clbuf_create_cst( &m->context, &di->src_recs.cl_src_pos);
            __GUARD clbuf_create_cst( &m->context, &di->src_recs.cl_rec_pos);
            __GUARD clbuf_create_cst( &m->context, &di->src_recs.cl_src);
            if (m->GRADSRCOUT){
                di->src_recs.cl_grad_src.size=sizeof(float)
                * m->NT * m->src_recs.nsmax;
                __GUARD clbuf_create_cst(&m->context,&di->src_recs.cl_grad_src);
            }
        }
        ...
        for (i=0;i<m->nvars;i++){
            ...
            if (di->vars[i].to_output){
                ...
                if (m->vars[i].gl_var_res){
                    di->vars[i].cl_var_res.size=sizeof(float)
                                               * m->NT * m->src_recs.ngmax;
                    __GUARD clbuf_create( &m->context, &di->vars[i].cl_var_res);
                }
            }
            ...
            if (m->GRADOUT
                ...
                di->vars[i].cl_fvar.size=sizeof(cl_float2)
