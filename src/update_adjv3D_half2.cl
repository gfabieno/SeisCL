...
FUNDEF void update_adjv(int offcomm,
                        ...
                        GLOBARG __gprec *gradrip, GLOBARG __gprec *gradrjp,
                        GLOBARG __gprec *gradrkp,
                        ...
                        LOCARG2)
{
    ...
    // Correct spatial derivatives to implement CPML for the adjoint variables
    #if ABS_TYPE==1
    {
        int i,j,k,indm, indn;
        if (DIV*gidz>DIV*NZ-NAB-FDOH-1){
            ...
            psi_syz_z[indn] = __f22h2(__hpgi(&b_z[indm+1]) * psi_syz_z[indn]
                                      + __hpgi(&a_z[indm+1]) * syzr_z2);
            syzr_z2 = syzr_z2 / __hpgi(&K_z[indm+1]) + psi_syz_z[indn];
            ...
            psi_syz_z[indn] = __f22h2(__hpg(&b_z[k]) * psi_syz_z[indn]
                                      + __hpg(&a_z[k]) * syzr_z2);
            syzr_z2 = syzr_z2 / __hpg(&K_z[k]) + psi_syz_z[indn];
        ...
        if (gidy-FDOH<NAB){
            ...
            psi_sxy_y[indn] = __f22h2(b_y[j] * psi_sxy_y[indn]
                                      + a_y[j] * sxyr_y2);
            sxyr_y2 = sxyr_y2 / K_y[j] + psi_sxy_y[indn];
            psi_syy_y[indn] = __f22h2(b_y_half[j] * psi_syy_y[indn]
                                      + a_y_half[j] * syyr_y1);
            syyr_y1 = syyr_y1 / K_y_half[j] + psi_syy_y[indn];
            psi_syz_y[indn] = __f22h2(b_y[j] * psi_syz_y[indn]
                                      + a_y[j] * syzr_y2);
            syzr_y2 = syzr_y2 / K_y[j] + psi_syz_y[indn];
        }
            ...
            psi_sxy_y[indn] = __f22h2(b_y[indm+1] * psi_sxy_y[indn]
                                      + a_y[indm+1] * sxyr_y2);
            sxyr_y2 = sxyr_y2 / K_y[indm+1] + psi_sxy_y[indn];
            psi_syy_y[indn] = __f22h2(b_y_half[indm] * psi_syy_y[indn]
                                      + a_y_half[indm] * syyr_y1);
            syyr_y1 = syyr_y1 / K_y_half[indm] + psi_syy_y[indn];
            psi_syz_y[indn] = __f22h2(b_y[indm+1] * psi_syz_y[indn]
                                      + a_y[indm+1] * syzr_y2);
            syzr_y2 = syzr_y2 / K_y[indm+1] + psi_syz_y[indn];
        }
        ...
        if (gidx-FDOH<NAB){
            ...
            psi_sxy_x[indn] = __f22h2(b_x[i] * psi_sxy_x[indn]
                                      + a_x[i] * sxyr_x2);
            sxyr_x2 = sxyr_x2 / K_x[i] + psi_sxy_x[indn];
        ...
        if (gidx>NX-NAB-FDOH-1){
            ...
            psi_sxy_x[indn] = __f22h2(b_x[indm+1] * psi_sxy_x[indn]
                                      + a_x[indm+1] * sxyr_x2);
            sxyr_x2 = sxyr_x2 / K_x[indm+1] + psi_sxy_x[indn];
    ...
    /* vx/vy/vz sit at the rip/rjp/rkp staggered positions (update_v3D.cl), so
     * their contributions are kept in separate accumulators -- NOT summed into
     * the cell-centred gradrho -- so average_grad_transpose() (calc_grad.c) can
     * apply the buoyancy averaging Jacobian to each separately. This mirrors
     * update_adjv2D_half2.cl's split and update_adjv3D.cl's FP32 one; this
     * kernel was simply missed when those were done, which made the FP16>=1 3D
     * density gradient come back about half its true value at EVERY cell, not
     * just near sources (measured 0.5702 at an interior cell against 1.0006 at
     * FP16==0). */
    gradrip[indp]=gradrip[indp] - scalefun(__h22f2c(lvx) * __h22f2c(lvxr),
                                           2*par_scale -src_scale - res_scale);
    gradrjp[indp]=gradrjp[indp] - scalefun(__h22f2c(lvy) * __h22f2c(lvyr),
                                           2*par_scale -src_scale - res_scale);
     ...
     * update_adjs2D_half2.cl for the vectorized form. lvxr/lvyr/lvzr are the
        ...
        for (int srci=0; srci<nsrc; srci++){
            ...
            if (st>=0 && st<=2){
                ...
                if (si==gidx && sj==gidy && (skf/DIV)==gidz){
                    ...
                    #endif
                    __gprec psi;
                    if      (st==0) psi = __h22f2(vxr[indv]);
                    else if (st==1) psi = __h22f2(vyr[indv]);
                    else            psi = __h22f2(vzr[indv]);
                    ...
                    else if (st==1) gradrjp[indp] = gradrjp[indp] + sc;
