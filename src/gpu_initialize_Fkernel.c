...
char *get_build_options(struct varcl *vcl, struct modcsts *m, int LCOMM, int comm, int DIRPROP)
{
    ...
    for (i=0;i<m->nvars;i++){
        sprintf(src,"-D %sout=%d ",m->vars[i].name,m->vars[i].to_output);
        strcat(build_options,src);
    }
    char src2[2000];
...
int create_kernel(cl_context  * context, cl_program  * program, cl_kernel * kernel, const char ** prog_source, char * prog_name, int nargs, char ** args, size_t *local_work_size, struct varcl *vcl, struct modcsts *m, int offcomm, int LCOMM, int comm, int DIRPROP){
    ...
    cl_err = create_gpu_kernel_from_string( *prog_source, program, context, kernel, prog_name, build_options);
    
    /*Define the size of the local variables of the compute device*/
    if ((*vcl).LOCAL_OFF==0 && local_work_size){
        for (i=0;i<m->NDIM;i++){
            shared_size*=local_work_size[i]+m->FDORDER;
        }
    }
...
//                printf("%s\n",args[i]);
        ...
        if (!argfound){
            for (j=0;j<m->nvars;j++){
                if (strcmp((*vcl).vars[j].name,args[i])==0){
                    cl_err = clSetKernelArg(*kernel,  i, sizeof(cl_mem), &(*vcl).vars[j].cl_var.mem);
                    argfound=1;
//                    printf("%s\n",args[i]);
                    break;
                }
        ...
        if (!argfound){
            for (j=0;j<m->ncsts;j++){
                if (strcmp((*vcl).csts[j].name,args[i])==0){
                    cl_err = clSetKernelArg(*kernel,  i, sizeof(cl_mem), &(*vcl).csts[j].cl_cst.mem);
                    argfound=1;
//                    printf("%s\n",args[i]);
                    break;
                }
        ...
        if (!argfound){
            if (strcmp("offcomm",args[i])==0){
                cl_err = clSetKernelArg(*kernel,  i, sizeof(int), &offcomm);
//                printf("%s\n",args[i]);
            }
        }
