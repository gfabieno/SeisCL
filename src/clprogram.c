...
int create_kernel(struct modcsts * m, struct varcl * vcl,  struct clprogram * prog){
    ...
    const char * build_options = get_build_options(vcl, m, prog->LCOMM, prog->COMM, prog->DIRPROP);
    ...
    for (i=0;i<(*prog).ninputs;i++){
        ...
        for (j=0;j<m->npars;j++){
            if (strcmp((*vcl).pars[j].name,(*prog).input_list[i])==0){
                cl_err = clSetKernelArg((*prog).kernel,  i, sizeof(cl_mem), &(*vcl).pars[j].cl_par.mem);
                argfound=1;
        ...
        if (!argfound){
            for (j=0;j<m->nvars;j++){
                ...
                if (strcmp(str2comp,(*prog).input_list[i])==0){
                    cl_err = clSetKernelArg((*prog).kernel,  i, sizeof(cl_mem), &(*vcl).vars[j].cl_varout.mem);
                    argfound=1;
                    break;
                }
                sprintf(str2comp,"%sbnd",(*vcl).vars[j].name);
                if (strcmp(str2comp,(*prog).input_list[i])==0){
                    cl_err = clSetKernelArg((*prog).kernel,  i, sizeof(cl_mem), &(*vcl).vars[j].cl_varbnd.mem);
                    argfound=1;
                    break;
                }
                sprintf(str2comp,"f%s",(*vcl).vars[j].name);
                if (strcmp(str2comp,(*prog).input_list[i])==0){
                    cl_err = clSetKernelArg((*prog).kernel,  i, sizeof(cl_mem), &(*vcl).vars[j].cl_fvar.mem);
                    argfound=1;
                    break;
                }
                sprintf(str2comp,"%s_buf1",(*vcl).vars[j].name);
                if (strcmp(str2comp,(*prog).input_list[i])==0){
                    cl_err = clSetKernelArg((*prog).kernel,  i, sizeof(cl_mem), &(*vcl).vars[j].cl_buf1.mem);
                    argfound=1;
                    break;
                }
                sprintf(str2comp,"%s_buf2",(*vcl).vars[j].name);
                if (strcmp(str2comp,(*prog).input_list[i])==0){
                    cl_err = clSetKernelArg((*prog).kernel,  i, sizeof(cl_mem), &(*vcl).vars[j].cl_buf2.mem);
                    argfound=1;
                    break;
                }
            }
        ...
        if (!argfound){
            if (strcmp("src" ,(*prog).input_list[i])==0){
                cl_err = clSetKernelArg((*prog).kernel,  i, sizeof(cl_mem), &(*vcl).src_recs.cl_src.mem);
                argfound=1;
            }
            else if (strcmp("src_pos" ,(*prog).input_list[i])==0){
                cl_err = clSetKernelArg((*prog).kernel,  i, sizeof(cl_mem), &(*vcl).src_recs.cl_src_pos.mem);
                argfound=1;
            }
            else if (strcmp("rec_pos" ,(*prog).input_list[i])==0){
                cl_err = clSetKernelArg((*prog).kernel,  i, sizeof(cl_mem), &(*vcl).src_recs.cl_rec_pos.mem);
                argfound=1;
            }
            else if (strcmp("grad_src" ,(*prog).input_list[i])==0){
                cl_err = clSetKernelArg((*prog).kernel,  i, sizeof(cl_mem), &(*vcl).src_recs.cl_grad_src.mem);
                argfound=1;
            }
        }
        ...
        if (!argfound){
            if (strcmp("offcomm",(*prog).input_list[i])==0){
                cl_err = clSetKernelArg((*prog).kernel,  i, sizeof(int), &prog->OFFCOMM);
                argfound=1;
                //                printf("%s\n",(*prog).input_list[i]);
            }
        }
