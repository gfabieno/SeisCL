...
int readhdf5(struct filenames files, struct modcsts * m) {
    ...
    float thisvp, thisvs, thistaup, thistaus;
    ...
    //Find vpmax and vsmin and transform variables in modulus
    if (!state){
        
        if (m->ND!=21){
            if (m->param_type==1){
                m->vpmax=sqrt(m->pi[0]/m->rho[0]);
                m->vsmin=sqrt(m->u[0]/m->rho[0]);
                ...
                for (i=0;i<m->NX*m->NY*m->NZ;i++){
                    ...
                    if (m->vpmax<thisvp) m->vpmax=thisvp;
                    if (m->vsmin>thisvs && thisvs>0) m->vsmin=thisvs;
                    
                }
            }
            if (m->param_type==2){
                m->vpmax=m->pi[0]/m->rho[0];
                m->vsmin=m->u[0]/m->rho[0];
                
                for (i=0;i<m->NX*m->NY*m->NZ;i++){
                    
                    thisvp=m->pi[i]/m->rho[i];
                    thisvs=m->u[i]/m->rho[i];
            ...
            if (m->param_type==3){
                m->vpmax=m->pi[0]-m->taup[0];
                m->vsmin=m->u[0]-m->taus[0];
            ...
            if (m->param_type==1){
                m->vpmax=sqrt(m->u[0]/m->rho[0]);
                m->vsmin=sqrt(m->u[0]/m->rho[0]);
                
                for (i=0;i<m->NX*m->NY*m->NZ;i++){
                    thisvs=sqrt(m->u[i]/m->rho[i]);
                    if (m->vpmax<thisvs) m->vpmax=thisvs;
                    if (m->vsmin>thisvs && thisvs>0) m->vsmin=thisvs;
                    
                }
            }
            if (m->param_type==2){
                m->vpmax=m->u[0]/m->rho[0];
                m->vsmin=m->u[0]/m->rho[0];
