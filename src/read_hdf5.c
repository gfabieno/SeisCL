...
int readhdf5(struct filenames files, struct modcsts * m) {
    ...
    dims2D[0]=1;dims2D[1]=m->numdim;
    if (m->numdim>MAX_DIMS) {state=1;fprintf(stderr, "Maximum number of dimensions is %d\n",MAX_DIMS);};
    ...
    for (i=0;i<m->nparams;i++){
        if (m->params[i].to_read){
            __GUARD checkexists(file_id,m->params[i].to_read);
            GMALLOC(m->params[i].gl_param,sizeof(float)*m->params[i].num_ele)
            if (!state) if ((state=checkmatndim(file_id, m->params[i].to_read, m->numdim, dimsND)))  {state=1;fprintf(stderr, "Variable %s must have dimensions given by N\n",m->params[i].to_read);};
            __GUARD readvar(file_id, H5T_NATIVE_FLOAT, m->params[i].to_read, m->params[i].gl_param);
        }
    }
